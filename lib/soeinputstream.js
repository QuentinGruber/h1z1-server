            ...
            for (var i=1;i<65536;i++) {
                ...
                if (fragment) {
                    ...
                    if (dataSize > totalSize) {
                        throw "processDataFragments: offset > totalSize: " + dataSize + " > " + totalSize + " (sequence " + j + ") (fragment length " + fragment.length + ")";
                    }
                    if (dataSize == totalSize) {
...
SOEInputStream.prototype.write = function(data, sequence, fragment) {
    ...
    //console.log(sequence, this._nextSequence);
    if (sequence > this._nextSequence) {
        debug("Sequence out of order, expected " + this._nextSequence + " but received " + sequence);
