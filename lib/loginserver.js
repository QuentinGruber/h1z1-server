...
function LoginServer(gameId, environment, serverPort, loginKey, backend) {
	...
	var mongoClient = (this._mongoClient = new MongoClient(new MongoServer('localhost', 27017), {
		native_parser: true
	}));
	...
	soeServer.on('disconnect', function(err, client) {
		...
		console.log('Client disconnected from ' + client.address + ':' + client.port);
		server.emit('disconnect', err, client);
	});

	soeServer.on('session', function(err, client) {
		debug('Session started for client ' + client.address + ':' + client.port);
		console.log('Session started for client ' + client.address + ':' + client.port);
	});
...
LoginServer.prototype.start = function(callback) {
	...
	this._mongoClient.open(function(err, mongoClient) {
		...
		server._soeServer.start(server._compression, server._crcSeed, server._crcLength, server._udpLength);
	});
};
