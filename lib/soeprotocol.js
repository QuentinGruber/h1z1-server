...
var packets = [
    ...
    0x01,
    {
      parse: function (data, crcSeed, compression, isSubPacket) {
        var packetType = data.readUInt16BE(0),
          ...
          protocol = "LoginUdp_9"; // Hack TODO: fix that
        // protocol = data.readNullTerminatedString(14); Don't know how to fix that
        return {
      ...
      pack: function (packet, crcSeed, compression, isSubPacket) {
        var data = new Buffer(14 + packet.protocol.length + 1);
  ...
  [
    ...
    0x03,
    {
      ...
      pack: function (packet, crcSeed, compression, isSubPacket) {
        var dataParts = [],
          subData,
          data = new Buffer(2 + (compression ? 1 : 0));
  ...
  [
    ...
    0x11,
    {
      ...
      pack: function (packet, crcSeed, compression, isSubPacket) {
        var data = new Buffer(4 + (compression && !isSubPacket ? 1 : 0)),
          offset = 0;
...
function packSOEPacket(packetName, object, crcSeed, compression, isSubPacket) {
  ...
  if (packet) {
    if (packet.pack) {
      ...
      console.log("pack()", "No pack function for packet " + packet.name);
    }
  } else {
    console.log(
      "pack()",
      "Unknown or unhandled SOE packet type: " + packetType
    );
  }
  return data;
}
...
function parseSOEPacket(data, crcSeed, compression, isSubPacket, appData) {
  ...
  if (packet) {
    if (packet.parse) {
      ...
      console.log("parse()", "No parser for packet " + packet.name);
    }
  } else {
    console.log(
      "parse()",
