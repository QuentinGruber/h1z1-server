...
// ======================================================================
var EventEmitter = require("events").EventEmitter, util = require("util"), fs = require("fs"), dgram = require("dgram"), debug = require("debug")("TransparentProxy");
function TransparentProxy(remoteAddress, remotePort, localPort, localClientPort) {
    ...
    server.on("message", function (data, remote) {
        ...
        client.send(data, 0, data.length, me._serverPort, me._serverAddress, function (err, bytes) { });
    });
    client.on("message", function (data, remote) {
        ...
        server.send(data, 0, data.length, proxyClient.port, proxyClient.address, function (err, bytes) { });
    });
    client.on("listening", function () {
        var address = this.address();
        debug("Listening for remote server on " + address.address + ":" + address.port);
    });
    server.on("listening", function () {
        var address = this.address();
        debug("Listening for local client on " + address.address + ":" + address.port);
    });
}
