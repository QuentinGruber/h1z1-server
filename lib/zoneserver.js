...
function ZoneServer(serverPort, gatewayKey, UsingMongo) {
  ...
  console.log("Gateway server listening on 0.0.0.0:" + serverPort);
...
ZoneServer.prototype.start = async function (callback) {
  console.log("Starting zone server");
  this._startTime = Date.now();
  var me = this;
  if (this._usingMongo) {
    // for unknow reason if you doesn't use mongo the server stop running
