...
var packetHandlers = {
        ...
        if (client.character.currentLoadout) {
            ...
            for (var i = 0; i < loadoutSlots.length; i++) {
                if (loadoutSlots[i].loadoutSlotId == loadoutSlotId) {
                    ...
                    server.data("item_line_members").findOne({ itemLineId: itemLineId, itemLineIndex: 0 }, function (err, itemLineMember) {
                        ...
                        for (var j = 0; j < inventoryItems.length; j++) {
                            if (inventoryItems[j].itemData.baseItem.itemId == itemId) {
                                client.character.currentLoadoutSlotItem = inventoryItems[j].itemData;
                                break;
                            }
                        }
                    });
    ...
    "Command.ExecuteCommand": function (server, client, packet) {
        ...
        if (packet.data.commandHash == Jenkins.oaat("STAT")) {
            if (args[0] && args[1]) {
                server.data("string_hash_to_value").findOne({ string: "StatId." + args[0] }, function (err, statId) {
                    if (err || !statId) {
                        server.sendChatText(client, "No such stat string");
                        return;
                    }
                    server.data("character_stats").findOne({ id: statId.value }, function (err, stat) {
                        if (err || !statId) {
        ...
        if (packet.data.commandHash == Jenkins.oaat("WEAPONSTAT")) {
            if (client.character.currentLoadoutSlotItem) {
                var weaponItem = client.character.currentLoadoutSlotItem;
                server.data("client_item_datasheet_data").findOne({ itemId: weaponItem.baseItem.itemId }, function (err, clientItem) {
                    var weaponId = clientItem.weaponId;
                    server.data("weapon_definitions").findOne({ weaponId: weaponId }, function (err, weapon) {
                        server.data("fire_groups").findOne({ fireGroupId: weapon.fireGroups[0] }, function (err, fireGroup) {
                            var fireModes = fireGroup.fireModes;
                            switch (args[0]) {
                                case "FireMode.DefaultZoom":
                                    if (args[1]) {
                                        server.sendChatText(client, "Setting FireMode.DefaultZoom = " + parseFloat(args[1]));
                                ...
                                case "FireMode.RecoilMaxXModifier":
                                    if (args[1]) {
                                        server.sendChatText(client, "Setting FireMode.RecoilMaxXModifier = " + parseFloat(args[1]));
                                        for (var i = 0; i < fireModes.length; i++) {
                                            server.sendWeaponPacket(client, "Weapon.StatUpdate", {
                                                "statData": [{
                                                        "guid": weaponItem.baseItem.unknownGuid1,
                                ...
                                case "FireMode.RecoilMinXModifier":
                                    server.sendChatText(client, "Setting FireMode.RecoilMinXModifier = " + parseFloat(args[1]));
                                    if (args[1]) {
                                        for (var i = 0; i < fireModes.length; i++) {
                                            server.sendWeaponPacket(client, "Weapon.StatUpdate", {
                                                "statData": [{
                                ...
                                case "FireMode.RecoilAngleMin":
                                    if (args[1]) {
                                        server.sendChatText(client, "Setting FireMode.RecoilAngleMin = " + parseFloat(args[1]));
                                        for (var i = 0; i < fireModes.length; i++) {
                                            server.sendWeaponPacket(client, "Weapon.StatUpdate", {
                                                "statData": [{
                                                        "guid": weaponItem.baseItem.unknownGuid1,
                                ...
                                case "FireMode.RecoilAngleMax":
                                    if (args[1]) {
                                        server.sendChatText(client, "Setting FireMode.RecoilAngleMax = " + parseFloat(args[1]));
                                        for (var i = 0; i < fireModes.length; i++) {
                                            server.sendWeaponPacket(client, "Weapon.StatUpdate", {
                                                "statData": [{
                                                        "guid": weaponItem.baseItem.unknownGuid1,
                                ...
                                case "FireMode.CofRecoil":
                                    if (args[1]) {
                                        server.sendChatText(client, "Setting FireMode.CofRecoil = " + parseFloat(args[1]));
                                        for (var i = 0; i < fireModes.length; i++) {
                                            server.sendWeaponPacket(client, "Weapon.StatUpdate", {
                                                "statData": [{
                                                        "guid": weaponItem.baseItem.unknownGuid1,
                                ...
                                case "FireMode.CofMinScalar":
                                    if (args[1]) {
                                        server.sendChatText(client, "Setting FireMode.CofMinScalar = " + parseFloat(args[1]));
                                        for (var i = 0; i < fireModes.length; i++) {
                                            server.sendWeaponPacket(client, "Weapon.StatUpdate", {
                                                "statData": [{
                                                        "guid": weaponItem.baseItem.unknownGuid1,
                                ...
                                case "FireMode.CofScalar":
                                    if (args[1]) {
                                        server.sendChatText(client, "Setting FireMode.CofScalar = " + parseFloat(args[1]));
                                        for (var i = 0; i < fireModes.length; i++) {
                                            server.sendWeaponPacket(client, "Weapon.StatUpdate", {
                                                "statData": [{
                                                        "guid": weaponItem.baseItem.unknownGuid1,
                                ...
                                case "FireMode.RecoilMagnitudeModifier":
                                    if (args[1]) {
                                        server.sendChatText(client, "Setting FireMode.RecoilMagnitudeModifier = " + parseFloat(args[1]));
                                        for (var i = 0; i < fireModes.length; i++) {
                                            server.sendWeaponPacket(client, "Weapon.StatUpdate", {
                                                "statData": [{
                                                        "guid": weaponItem.baseItem.unknownGuid1,
                                ...
                                case "FireMode.CofScalarMoving":
                                    if (args[1]) {
                                        server.sendChatText(client, "Setting FireMode.CofScalarMoving = " + parseFloat(args[1]));
                                        for (var i = 0; i < fireModes.length; i++) {
                                            server.sendWeaponPacket(client, "Weapon.StatUpdate", {
                                                "statData": [{
                                                        "guid": weaponItem.baseItem.unknownGuid1,
                                ...
                                case "FireGroup.ProjectileSpeedMultiplier":
                                    if (args[1]) {
                                        server.sendChatText(client, "Setting FireGroup.ProjectileSpeedMultiplier = " + parseFloat(args[1]));
                                        server.sendWeaponPacket(client, "Weapon.StatUpdate", {
                                            "statData": [{
                                                    "guid": weaponItem.baseItem.unknownGuid1,
                                                    "unknownBoolean1": false,
                                ...
                                case "FireMode.ProjectileOverride":
                                    if (args[1]) {
                                        server.sendChatText(client, "Setting FireMode.ProjectileOverride = " + parseInt(args[1], 10));
                                        for (var i = 0; i < fireModes.length; i++) {
                                            server.sendWeaponPacket(client, "Weapon.StatUpdate", {
                                                "statData": [{
                                                        "guid": weaponItem.baseItem.unknownGuid1,
                                ...
                                case "FireMode.RecoilHorizontalTolerance":
                                    if (args[1]) {
                                        server.sendChatText(client, "Setting FireMode.RecoilHorizontalTolerance = " + parseFloat(args[1]));
                                        for (var i = 0; i < fireModes.length; i++) {
                                            server.sendWeaponPacket(client, "Weapon.StatUpdate", {
                                                "statData": [{
                                                        "guid": weaponItem.baseItem.unknownGuid1,
                                ...
                                case "FireMode.RecoilHorizontalMin":
                                    if (args[1]) {
                                        server.sendChatText(client, "Setting FireMode.RecoilHorizontalMin = " + parseFloat(args[1]));
                                        for (var i = 0; i < fireModes.length; i++) {
                                            server.sendWeaponPacket(client, "Weapon.StatUpdate", {
                                                "statData": [{
                                                        "guid": weaponItem.baseItem.unknownGuid1,
                                ...
                                case "FireMode.RecoilHorizontalMax":
                                    if (args[1]) {
                                        server.sendChatText(client, "Setting FireMode.RecoilHorizontalMax = " + parseFloat(args[1]));
                                        for (var i = 0; i < fireModes.length; i++) {
                                            server.sendWeaponPacket(client, "Weapon.StatUpdate", {
                                                "statData": [{
                                                        "guid": weaponItem.baseItem.unknownGuid1,
                                ...
                                case "FireGroup.ProjectileSpeedOverride":
                                    if (args[1]) {
                                        server.sendChatText(client, "Setting FireGroup.ProjectileSpeedOverride = " + parseFloat(args[1]));
                                        server.sendWeaponPacket(client, "Weapon.StatUpdate", {
                                            "statData": [{
                                                    "guid": weaponItem.baseItem.unknownGuid1,
                                                    "unknownBoolean1": false,
                                ...
                                case "AmmoSlot.ClipSize":
                                    if (args[1]) {
                                        server.sendChatText(client, "Setting AmmoSlot.ClipSize = " + parseInt(args[1], 10));
                                        server.sendWeaponPacket(client, "Weapon.StatUpdate", {
                                            "statData": [{
                                                    "guid": weaponItem.baseItem.unknownGuid1,
                                                    "unknownBoolean1": false,
    ...
    "AdminCommand.SpawnVehicle": function (server, client, packet) {
        var guid = server.generateGuid(), transientId = server.getTransientId(client, guid);
        server.data("vehicles").findOne({ id: packet.data.vehicleId }, function (err, vehicle) {
            if (err || !vehicle) {
                server.sendChatText(client, "No such vehicle");
                return;
            }
            server.data("npc_vehicle_mappings").findOne({ vehicle_id: packet.data.vehicleId }, function (err, npcDefinitionMapping) {
                if (err || !npcDefinitionMapping) {
                    server.sendChatText(client, "Vehicle has no NPC mapping");
                    return;
                }
                server.data("npcs").findOne({ id: npcDefinitionMapping.npc_definition_id }, function (err, npc) {
                    if (err || !npc) {
                        server.sendChatText(client, "NPC definition " + npcDefinitionMapping.npc_definition_id + " not found");
                        return;
                    }
    ...
    "ProfileStats.GetPlayerProfileStats": function (server, client, packet) {
        server.sendData(client, "ProfileStats.PlayerProfileStats", JSON.parse(fs.readFileSync(__dirname + "/data/zone/profilestats.json")));
    },
