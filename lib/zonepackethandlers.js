...
var packetHandlers = {
  ClientIsReady: function (server, client, packet) {
    ...
    server.sendData(client, "QuickChat.SendData", {
      commands: [],
    });
    ...
    server.sendData(client, "ActivityManager.ProfileActivityList", {
      stats: [],
    });
      ...
      for (var i = 0; i < loadoutSlots.length; i++) {
        if (loadoutSlots[i].loadoutSlotId == loadoutSlotId) {
          ...
          server
            ...
            .findOne({ itemLineId: itemLineId, itemLineIndex: 0 }, function (
              err,
              itemLineMember
            ) {
  ...
  "Command.ExecuteCommand": function (server, client, packet) {
    ...
    if (packet.data.commandHash == Jenkins.oaat("LOG")) {
      var _gatewayServer = server._gatewayServer,
    ...
    if (packet.data.commandHash == Jenkins.oaat("WEAPONSTAT")) {
      if (client.character.currentLoadoutSlotItem) {
        ...
        server
          ...
          .findOne({ itemId: weaponItem.baseItem.itemId }, function (
            err,
            clientItem
          ) {
            ...
            server
              ...
              .findOne({ weaponId: weaponId }, function (err, weapon) {
                server
                  .data("fire_groups")
                  .findOne({ fireGroupId: weapon.fireGroups[0] }, function (
                    err,
                    fireGroup
                  ) {
                    ...
                    switch (args[0]) {
                      ...
                      case "PelletsPerShot":
                        if (args[1]) {
                          server.sendChatText(
                            client,
                            "Setting PelletsPerShot = " + parseFloat(args[1])
                          );
                          for (var i = 0; i < fireModes.length; i++) {
                            server.sendWeaponPacket(
                              client,
    ...
    server
      ...
      .findOne({ id: packet.data.vehicleId }, function (err, vehicle) {
        ...
        server
          ...
          .findOne({ vehicle_id: packet.data.vehicleId }, function (
            err,
            npcDefinitionMapping
          ) {
