var EventEmitter = require("events").EventEmitter,
  SOEProtocol = require("./soeprotocol").SOEProtocol,
  SOEPackets = require("./soeprotocol").SOEPackets,
...
function SOEServer(protocolName, serverPort, cryptoKey, compression) {
  ...
  function handlePacket(client, packet) {
    ...
    if (result != null) {
      switch (soePacket.name) {
        case "SessionRequest":
          console.log("Received session request from " + client.address);
        ...
        case "MultiPacket":
          ...
          break;
        case "Ping": // TODO maybe respond to it ?
          console.log("Received ping from client");

          break;
        case "NetStatusRequest":
          console.log("Received net status request from client");
          break;
        case "Data":
          console.log(
        ...
        case "OutOfOrder":
          ...
          break;
        case "Ack":
          console.log("Ack, sequence " + result.sequence);
          client.outputStream.ack(result.sequence);
          break;
        case "FatalError":
          console.log("Received fatal error from client");
          break;
  ...
  connection.on("message", function (data, remote) {
    console.log(data.length + " bytes from client");
    var client;
    if (!clients[remote.address]) {
      ...
      var checkOutOfOrderQueue = function () {
        if (client.outOfOrderPackets.length) {
          ...
          console.log("Sending " + packets.length + " OutOfOrder packets");
          me._sendPacket(
            client,
            "MultiPacket",
            {
...
SOEServer.prototype.sendAppData = function (client, data, overrideEncryption) {
  console.log("Sending app data: " + data.length + " bytes");
  client.outputStream.write(data, overrideEncryption);
};

SOEServer.prototype.toggleEncryption = function (value) {
  value = !!value;
  this._useEncryption = value;
  console.log(this._guid, "Toggling encryption: value = " + value);
  for (var i in this._clients) {
    if (this._clients.hasOwnProperty(i)) {
      var client = this._clients[i];
      client.outputStream.toggleEncryption(value);
      client.inputStream.toggleEncryption(value);
    }
  }
};
