    ...
    function handlePacket(client, packet) {
        ...
        switch (soePacket.name) {
            ...
            case "MultiPacket":
                ...
                if (lastOutOfOrder > 0) {
                    debug("Received multiple out-order-packet packet on channel " + channel + ", sequence " + lastOutOfOrder);
            ...
            case "DataFragment":
                ...
                break;
            case "OutOfOrder":
                debug("Received out-order-packet packet on channel " + result.channel + ", sequence " + result.sequence);
...
SOEServer.prototype._sendPacket = function(client, packetName, packet, prioritize) {
    ...
    throw packetName + " " + client.crcSeed + " " + client.compression + " " + JSON.stringify(packet, null, 4);
}
    if (prioritize) {
