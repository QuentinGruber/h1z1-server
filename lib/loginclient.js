...
function LoginClient(
  ...
  soeClient.on("appdata", function (err, data) {
    ...
    try {
      ...
      console.log("Failed parsing app data loginclient_appdata_" + n + ".dat");
      return;
    }
  ...
  soeClient.on("connect", function (err, result) {
    ...
    function sendLoginRequest(sessionId) {
      me._sessionId = sessionId.toString();
      me._fingerprint = "fingerprint";
      me._protocol = new LoginProtocol();
      debug("Sending login request");
      console.log("Sending login request");
      var data = me._protocol.pack("LoginRequest", {
        sessionId: me._sessionId,
        systemFingerPrint: me._fingerprint,
      });
      soeClient.sendAppData(data, true);
    }
    if (me._sessionId) {
      sendLoginRequest(me._sessionId);
    } else {
...
LoginClient.prototype.requestCharacterLogin = function (
  ...
  console.log("Requesting character login");
  ...
  if (data) {
    ...
    console.log("Could not pack character login request data");
  }
};
...
LoginClient.prototype.getPlaySession = function (token, callback) {
  ...
  console.log("Fetching play session from lp.soe.com");
  var me = this;
  var options = {
    host: "lp.soe.com",
  ...
  var request = https.get(options, function (res) {
    ...
    res.on("end", function () {
      ...
      if (obj && obj.result == "SUCCESS") {
        ...
        console.log("Received play session (" + sessionId + ")");
        callback(null, sessionId);
      } else {
        debug("Play session request failed");
        console.log("Play session request failed");
