...
export class SOEInputStream {
  ...
  write(data: Uint8Array, sequence: number, fragment: any): void {
    ...
    this._fragments[sequence] = Buffer.from(data);
    if (!fragment) {
...
const inputStream = new SOEInputStream(workerData.cryptoKey as string);
...
parentPort?.on("message", (message: Message) => {
  switch (message.type) {
    case "setEncryption":{
      const { data } = message;
      inputStream.setEncryption(data);
      break;
    }
    case "toggleEncryption":{
      inputStream.toggleEncryption();
      break;
    }
    case "write":{
      const { data:{data,sequence,fragment} } = message;
      inputStream.write(data,sequence,fragment)
      break;
    }
    case "processDataFragments":{
      inputStream._processDataFragments();
      break;
    }
    case "kill":{
      process.exit(0);
