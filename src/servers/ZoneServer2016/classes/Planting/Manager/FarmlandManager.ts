...
export class FarmlandManager {
  ...
  private readonly _fertilizerExpirationTimers: {
  ...
  constructor(
    ...
    this._charactersFurrows = charactersFurrowsData
  ...
  public Reclaim = (client: Client, server: ZoneServer2016): boolean => {
    ...
    if (pos) {
      ...
      const newFurrows = new Furrows(
        client.character.characterId,
        pos,
        new Euler(rot[0], 0, 0),
        Date.now(),
        this._setting.DefaultFurrowsDuration,
        [],
        server.generateGuid()
      );
  ...
  public ReUseFurrows = (furrows: Furrows, cropDuration: number) => {
    ...
    if (guid) {
      ...
      this.setFurrowsTimeout(
        furrows,
        this._setting.DefaultFurrowsDuration + cropDuration
      );
  ...
  //is simulating~~~~~~~~
  public SimulateGetSurroundingSowAbleFurrows = (
    client: Client,
    //false is get role surrounding
    bySightPoint: boolean
  ): Furrows | null => {
    let usePos = new Vector4(
    ...
    const fs = this.searchTiledFurrowsListAroundPosition(
      client.character.characterId,
      usePos,
  ...
  public GetSurroundingFertilizeAbleHoles = (
    client: Client,
    circleRadius: number
  ): Hole[] => {
    const fs = this.searchTiledFurrowsListAroundPosition(
  ...
  public BuryFertilizerIntoHole = (hole: Hole) => {
    if (
      ...
      (hole.Id && this._fertilizerExpirationTimers[hole.Id])
    )
      return false;
    this.setFertilizerInHoleTimeout(
      hole,
      this._setting.DefaultFertilizerDuration
    );
  ...
  public ReFertilizeHole = (hole: Hole): boolean => {
    ...
    if (!this._fertilizerExpirationTimers[guid]) return false;
    clearTimeout(this._fertilizerExpirationTimers[guid]);
  ...
  private static calcLookAtPosition(client: Client): Vector4 | null {
    ...
    return getLookAtPos(
      new Vector4(pos[2], pos[1], pos[0], 1),
      -euler[0],
      euler[2],
      euler[1],
      roleHeight
  ...
  private searchTiledFurrowsListAroundPosition(
    characterId: string,
    sightPoint: Vector4,
    circleRadius: number
  ): Furrows[] {
  ...
  private triggerRemoveFertilizerFromHoleEffect = (hole: Hole) => {
    if (!this._server) {
      debug(
        "need to trigger the effect of fertilizer removal from the hole: ",
        hole
      );
      return false;
    }
