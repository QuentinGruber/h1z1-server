...
export class GrowingManager {
  ...
  constructor(private _setting: PlantingSetting) {
    Object.entries(defaultTestGrowthScripts).forEach(([k, v]) => {
      if (!_setting.GrowthScripts[k]) _setting.GrowthScripts[k] = v;
  ...
  public StartCultivating = (
    ...
    if (hole && seed) {
      if (this._setting.GrowthScripts[seed.Name]) {
        ...
        if (stagesKeys.length < 1) return false;
        const firstStage = script.Stages[stagesKeys[0]];
        hole.InsideCropsPile = new CropsPile(
          seed,
          CropsPileStatus.Sowed,
          seed.Guid
        );
        return this.grow2Stage(
          client,
          server,
          hole,
          seed.Name,
  ...
  public AccelerateGrowth = (
    ...
    if (!hole.InsideCropsPile) return false;
    const guid = hole.Id;
    ...
    if (this._stageTimers[guid]) {
      this.grow2Stage(
        ...
        server,
        hole,
        this._stageTimers[guid].scriptName,
        this._stageTimers[guid].srcStage,
        this._stageTimers[guid].destStage
      );
    }
  };

  public PickingMatureCrops = (
    hole: Hole,
    client: ZoneClient2016,
    server: ZoneServer2016
  ): boolean => {
    if (hole.InsideCropsPile) {
      if (hole.InsideCropsPile.LootAbleProducts.length) {
        for (const lootAbleProduct of hole.InsideCropsPile.LootAbleProducts) {
          const randomOutcomeItemByScript = server.generateItem(
  ...
  private grow2Stage = (
    ...
    if (hole.LastFertilizeTime && hole.FertilizerDuration) {
      ...
      const timeToReachByAcceleration =
        destStage.TimeToReach / this._setting.FertilizerAcceleration;
      ...
      //如果本来需要10秒,有化肥的加成以后,就用5秒,化肥实际可用时间是4秒,前面的8秒时间被化肥加成所用4秒,后面的时间正常.那最后的结果是6秒
        ...
        // 这剩余的4秒可以产生8秒的效果
        const quick =
          fertilizerRemaining * this._setting.FertilizerAcceleration;
        // 还剩下2秒正常的
    ...
    const timer = setTimeout(() => {
      ...
      if (destStage.Outcome) {
        for (let i = 0; i < destStage.Outcome.length; i++) {
          ...
          const count = this.calcGettingCount(
            current.DefiniteCount,
            current.MinCount,
            current.MaxCount,
            current.RateOfGetting
          );
          if (count) {
      ...
      if (sts.length < nextIndex + 1) {
        //no more
        debug(
          "没有更多状态了.no more stage,the end, now you can loot crops if correct!!"
        );
        return;
      } else {
        const nextStage =
          this._setting.GrowthScripts[scriptName].Stages[sts[nextIndex]];
        this.grow2Stage(client, server, hole, scriptName, destStage, nextStage);
      }
    }, realTimeToReach);
    this._stageTimers[guid] = {
      timer: timer,
      destStage: destStage,
      srcStage: srcStage,
  ...
  private calcGettingCount = (
    ...
    if (definiteCount !== undefined) {
      ...
      if (rateOfGetting !== undefined) {
        ...
        return rateOfGetting >= percent
          ? randomIntFromInterval(minCount, maxCount)
          : 0;
      } else {
  ...
  private static changeCropsModel(
    hole: Hole,
    modelId: number,
    server: ZoneServer2016
  ): boolean {
    ...
    // );
    const eid =
      hole.LastFertilizeTime &&
      hole.FertilizerDuration &&
      Date.now() - hole.LastFertilizeTime > 0
        ? 5056
        : 0;
    server.sendDataToAllWithSpawnedEntity(
      server._spawnedItems,
