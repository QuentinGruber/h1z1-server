...
const GetCrossDotOnPlane = (Lx1: number, Ly1: number, Lz1: number, Lx2: number, Ly2: number, Lz2: number, Px1: number, Py1: number, Pz1: number, Px2: number, Py2: number, Pz2: number, Px3: number, Py3: number, Pz3: number): Vector4 | null => {
    ...
    const A = Py1 * Pz2 + Py2 * Pz3 + Py3 * Pz1 - Py1 * Pz3 - Py2 * Pz1 - Py3 * Pz2;
    const B = -(Px1 * Pz2 + Px2 * Pz3 + Px3 * Pz1 - Px3 * Pz2 - Px2 * Pz1 - Px1 * Pz3);
    const C = Px1 * Py2 + Px2 * Py3 + Px3 * Py1 - Px1 * Py3 - Px2 * Py1 - Px3 * Py2;
    const D = -(Px1 * Py2 * Pz3 + Px2 * Py3 * Pz1 + Px3 * Py1 * Pz2 - Px1 * Py3 * Pz2 - Px2 * Py1 * Pz3 - Px3 * Py2 * Pz1);
...
export const getLookAtPos = (standPos: Vector4, yaw: number, pitch: number, roll: number, roleHeight: number): Vector4 | null => {
    ...
    const cameraPos = {X: rolePos.X, Y: rolePos.Y + roleHeight, Z: rolePos.Z, W: 1};
    ...
    const cameraLookAtOnePos = new Vector4(cameraPos.X + cameraNewDirect.X, cameraPos.Y + cameraNewDirect.Y, cameraPos.Z + cameraNewDirect.Z, 0);
