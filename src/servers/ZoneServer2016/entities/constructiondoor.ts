...
export class ConstructionDoor extends DoorEntity {
  ...
  OnPlayerSelect(server: ZoneServer2016, client: ZoneClient2016, isInstant?: boolean) {
    ...
    if (isInstant) {
        if (this.passwordHash == 0 || this.grantedAccess.includes(client.character.characterId) || client.character.characterId === this.ownerCharacterId) {
            if (this.moving) {
                return;
            }
            this.moving = true;
  ...
  OnInteractionString(server: ZoneServer2016, client: ZoneClient2016) {
    ...
    if (client.character.characterId === this.ownerCharacterId || !this.grantedAccess.includes(client.character.characterId)) {
