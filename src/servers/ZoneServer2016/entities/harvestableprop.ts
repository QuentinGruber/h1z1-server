...
import { ContainerLootSpawner, DamageInfo, LootDefinition } from "types/zoneserver";
...
function getModelId(actorDefinition: string): {modelId: number, nameId: number, lootSpawner: string} {
    switch (actorDefinition) {
        case "Common_Props_Harvestable_SUV_proxy.adr":
            return { modelId: ModelIds.Common_Props_AbandonedSUV_Harvestable, nameId: StringIds.WRECKED_VAN, lootSpawner: "Harvestable Vehicle" }
        case "Common_Props_Harvestable_Sedan_proxy.adr":
            return { modelId: ModelIds.Common_Props_AbandonedSedan_Harvestable, nameId: StringIds.WRECKED_SEDAN, lootSpawner: "Harvestable Vehicle" }
        case "Common_Props_Harvestable_PickupTruck_proxy.adr":
            return { modelId: ModelIds.Common_Props_AbandonedTruck_Harvestable, nameId: StringIds.WRECKED_TRUCK, lootSpawner: "Harvestable Truck" }
        default:
...
export class HarvestableProp extends BaseLootableEntity {
        ...
        if (this.stage == 2) return;
            server.utilizeHudTimer(
                ...
                () => {
                    ...
                    if (!weapon2 || weapon2.itemDefinitionId != Items.WEAPON_CROWBAR) return
                    ...
                    switch (this.actorModelId) {
                        ...
                        case ModelIds.Common_Props_AbandonedSedan_Harvestable_Stage01:
                            switch (this.stage) {
                                case 1:
                                    this.actorModelId = ModelIds.Common_Props_AbandonedSedan_Harvestable_Stage01
                                    effectId = Effects.PFX_Harvest_Abandoned_Sedan_Stage1
                                    break;
                                case 2:
                                    this.actorModelId = ModelIds.Common_Props_AbandonedSedan_Harvestable_Stage02;
                        ...
                        case ModelIds.Common_Props_AbandonedSUV_Harvestable_Stage01:
                            switch (this.stage) {
                                case 1:
                                    this.actorModelId = ModelIds.Common_Props_AbandonedSUV_Harvestable_Stage01
                                    effectId = Effects.PFX_Harvest_Abandoned_SUV_Stage1
                                    break;
                                case 2:
                                    this.actorModelId = ModelIds.Common_Props_AbandonedSUV_Harvestable_Stage02;
                        ...
                        case ModelIds.Common_Props_AbandonedTruck_Harvestable_Stage01:
                            switch (this.stage) {
                                case 1:
                                    this.actorModelId = ModelIds.Common_Props_AbandonedTruck_Harvestable_Stage01
                                    effectId = Effects.PFX_Harvest_Abandoned_Truck_Stage1
                                    break;
                                case 2:
                                    this.actorModelId = ModelIds.Common_Props_AbandonedTruck_Harvestable_Stage02;
                    ...
                    const lootSpawner = containerLootSpawnersBWC[this.lootSpawner] as ContainerLootSpawner;
                    lootSpawner.items.forEach((lootDefinition: LootDefinition) => {
                        ...
                        if (spawnChance <= lootDefinition.weight) {
                            const count = randomIntFromInterval(lootDefinition.spawnCount.min, lootDefinition.spawnCount.max)
    ...
    updateStage(server: ZoneServer2016, effectId: number) {
        server.sendDataToAllWithSpawnedEntity(server._lootableProps, this.characterId,
