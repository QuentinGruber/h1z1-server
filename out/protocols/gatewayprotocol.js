...
// ======================================================================
var debug = require("debug")("GatewayProtocol"), DataSchema = require("h1z1-dataschema"), GatewayPackets = require("../packets/gatewaypackets");
function GatewayProtocol() { }
GatewayProtocol.prototype.parse = function (data) {
    var packetType = data[0] & 0x1f;
    var result, schema, name;
    var packet = GatewayPackets.Packets[packetType];
    if (packet) {
...
GatewayProtocol.prototype.pack = function (packetName, object) {
    var packetType = GatewayPackets.PacketTypes[packetName], packet = GatewayPackets.Packets[packetType];
    var payload, data;
    if (packet) {
        if (packet.name === "TunnelPacketToExternalConnection" ||
            packet.name === "TunnelPacketFromExternalConnection") {
